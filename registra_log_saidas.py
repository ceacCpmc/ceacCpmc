...
def registrar(nome, cpf, email):
    try:
        ...
        dados = [cpf, nome, email, agora, 'Saída']
        ...
        return 'Usuário incluído no WKS'
    except Exception as erro:
        print('################')
        print(f' Erro na inclusão do WKS - {erro}')
        print('################')
        return 'Erro ao fazer logoff'

'''
conn3 = sqlite3.connect('acessos.sqlite')
cursor_logout = conn3.cursor()
...
dados_login_limpos = ""
dados_login = [nome, cpf, email, agora, 'Saida']
...
df = pd.read_sql('select * from logs', conn3)
